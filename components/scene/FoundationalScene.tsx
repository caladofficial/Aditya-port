      ...
      {Array.from(word).map((character, index) => (
        <span className="hero-letter-mask" key={`${character}-${index}`}>
          <motion.span
            ...
            initial={{ y: reduce ? "0%" : "108%", scale: reduce ? 1 : 1.035, opacity: reduce ? 1 : 0 }}
            animate={ready || reduce ? { y: "0%", scale: 1, opacity: 1 } : { y: "108%", scale: 1.035, opacity: 0 }}
            transition={{
              duration: reduce ? 0 : 0.76,
              delay: ready && !reduce ? 0.1 + line * 0.16 + index * 0.044 : 0,
              ease: eases.enter,
            }}
          >
            {character}
...
export function FoundationalScene() {
  ...
  // A failed browser animation should never leave the identity hidden.
  useEffect(() => {
        ...
        <section ref={heroRef} id="top" className="hero-scroll" data-fluid-scene="hero" aria-label="Aditya Rai introduction">
          <div className="hero-stage" onPointerMove={handlePointerMove} onPointerLeave={resetPointer}>
            ...
            >
              <motion.h1
                className="hero-name"
                aria-label="Aditya Rai"
                initial={{ filter: reduce ? "blur(0px)" : "blur(8px)" }}
                animate={{ filter: introReady || reduce ? "blur(0px)" : "blur(8px)" }}
                transition={{ duration: reduce ? 0 : 0.46, delay: introReady && !reduce ? 0.12 : 0, ease: eases.enter }}
